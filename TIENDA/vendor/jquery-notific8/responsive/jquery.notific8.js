...
(function ($) {
    ...
    methods = (function () {
        ...
        function buildNotification($this) {
            //EDIT: remueve una notificacion existente
            ...
            //EDIT: icon segun la clase
            if (!data.settings.icon)
                data.settings.icon = data.settings.theme == "success" ? "fa-check" : data.settings.theme == "error" ? "fa-trash-o" : "fa-exclamation-triangle";
            // check for an icon
            if (data.settings.hasOwnProperty('icon') && (typeof data.settings.icon === "string")) {
                ...
                sub_notification.append('<div class="hidden-xs col-sm-3 notific8-icon-wrapper"><i class="jquery-notific8-icon fa ' + data.settings.icon + '"></i></div>');
                sub_notification.append('<div id="sub_sub_notification" class="col-sm-9 col-xs-12 notific8-text-wrapper"></div>');
                sub_sub_notification = sub_notification.find("#sub_sub_notification");
            } else {
                notification.append("<div class='row'></div>");
                sub_notification = notification.find("div");
                sub_notification.append('<div id="sub_sub_notification" class="col-xs-12 has-no-icon notific8-text-wrapper"></div>');
                sub_sub_notification = sub_notification.find("#sub_sub_notification");                
            }
            // check for a heading
